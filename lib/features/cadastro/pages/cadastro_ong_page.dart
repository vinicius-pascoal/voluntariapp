...
class _CadastroOngPageState extends State<CadastroOngPage> {
  ...
  Future<void> cadastrar() async {
    ...
    if ([nome, cnpj, email, senha].any((value) => value.isEmpty)) {
    ...
    try {
      ...
      setState(() => mensagemErro = 'Erro inesperado ao cadastrar organização.');
    } finally {
  ...
  Future<void> loginGoogle() async {
    ...
    try {
      final credential = await AuthService().signInWithGoogle(defaultType: 'ong');
      if (!mounted) return;
      if (credential == null) {
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const _RequiredLabel('CNPJ'),
                          const SizedBox(height: 8),
                          _CadastroTextField(controller: cnpjController, keyboardType: TextInputType.number),
                          const SizedBox(height: 24),
